...
- 全面改进回放管理器界面，重做回放列表、详情、设置和更新页面，让操作更清晰。
- 新增判定难度记录与回放功能，记录当前使用的宽松、普通或严格判定难度，并可在回放详情中查看。

## 1.6.0 (2026-09-11)

- [UI] 优化管理器界面布局。
- [优化] 优化自定义谱面的进入与退出逻辑，简化编辑器下的回放流程。
- [系统] 增强运行稳定性并完善异常捕获机制。
- [新增] 新增教学关卡录制自动保存的配置项。
...
- **旧版兼容** — 可读取旧版手机版 `.rpl` 文件，实现移动端回放升级
...
- 旧版手机版 JSON `.rpl` 可读取，新文件保存为 `.rpl2`
