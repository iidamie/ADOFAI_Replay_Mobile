...
internal sealed record UiText(
    ...
    string DisableTutorialAutoSave,
    string DisableAutoReplay,
    string ShowHud,
    string ReceiveTouchInput,
    string ReceiveKeyboardInput,
    string HudSize,
    string MaxFiles,
    string Directory,
    string SaveCurrent,
    ...
    string PreviewUnavailable)
{
    ...
    private static readonly UiText Chinese = new(
        ...
        "次输入", "自动保存", "从第 1 砖完整通关时保存", "任意起点通关时保存", "每次失败时保存", "失败且进度达到 90% 时保存", "教学关不自动保存录制",
        "不录制自动模式", "显示回放状态", "接收触摸输入", "接收键盘输入", "状态字号", "最多保留回放", "回放目录", "保存当前记录", "保存回放", "正在保存", "回放已保存", "回放最近一局",
        "暂停", "继续", "停止回放", "打开回放管理器", "回放管理器", "回放", "搜索歌曲或作者", "回放文件", "刷新",
        "播放", "删除", "完成", "失败", "不支持", "没有回放文件", "当前没有可保存或回放的记录。", "操作已加入主线程队列。",
        "回放已保存", "无法加载回放", "关闭", "上一页", "下一页", "回放详情", "录制时间", "进度", "输入", "速度", "来源", "谱面路径",
        "官方谱面", "自定义谱面", "确认删除此回放？", "取消", "页", "艺术家",
        "回放标题", "保存标题", "标题已保存", "原始曲名", "返回列表", "谱面封面", "未提供可用的谱面封面或预览音乐");
    ...
    private static readonly UiText English = new(
        ...
        "inputs", "Auto save", "Save full clears from tile 1", "Save every completion", "Save every failure", "Save failures at 90% or later", "Do not auto-save tutorial levels",
        "Do not record autoplay", "Show replay status", "Receive touch input", "Receive keyboard input", "Status font size", "Maximum replay files", "Replay directory", "Save current attempt", "Save replay", "Saving", "Replay saved", "Replay latest attempt",
        ...
        "Replay saved", "Could not load replay", "Close", "Previous", "Next", "Replay details", "Recorded", "Progress", "Inputs", "Speed", "Source", "Level path",
        "Official level", "Custom level", "Delete this replay?", "Cancel", "Page", "Artist",
        "Replay title", "Save title", "Title saved", "Original song", "Back to list", "Chart preview", "No chart cover or preview audio is available");

    private static readonly UiText Korean = new(
        "녹화 중", "리플레이 불러오는 중", "게임 화면을 눌러 리플레이 시작", "리플레이 중", "리플레이 일시정지", "리플레이 종료", "실패 지점에서 종료",
        "회 입력", "자동 저장", "첫 타일부터 완주 저장", "모든 완주 저장", "모든 실패 저장", "90% 이후 실패 저장", "튜토리얼 레벨 리플레이 자동 저장 안 함",
        "오토플레이 녹화 안 함", "리플레이 상태 표시", "터치 입력 받기", "키보드 입력 받기", "상태 글자 크기", "최대 리플레이 수", "리플레이 폴더", "현재 기록 저장", "리플레이 저장", "저장 중", "리플레이 저장 완료", "최근 플레이 보기",
        ...
        "리플레이 저장 완료", "리플레이를 불러올 수 없습니다", "닫기", "이전", "다음", "리플레이 정보", "녹화 시간", "진행률", "입력", "속도", "출처", "레벨 경로",
        "공식 레벨", "커스텀 레벨", "이 리플레이를 삭제할까요?", "취소", "페이지", "아티스트",
        "리플레이 제목", "제목 저장", "제목 저장 완료", "원곡", "목록으로", "레벨 미리보기", "레벨 커버 또는 미리듣기 음악이 없습니다");

    private static readonly UiText Japanese = new(
        "記録中", "リプレイを読み込み中", "ゲーム画面をタップして開始", "リプレイ中", "一時停止中", "リプレイ終了", "失敗地点で終了",
        "入力", "自動保存", "最初からクリアを保存", "すべてのクリアを保存", "すべての失敗を保存", "90%以上での失敗を保存", "チュートリアルを自動保存しない",
        "オートプレイを記録しない", "状態を表示", "タッチ入力を受け取る", "キーボード入力を受け取る", "文字サイズ", "最大保存数", "保存先", "現在の記録を保存", "リプレイを保存", "保存中", "リプレイ保存済み", "直前のプレイを再生",
        "一時停止", "再開", "リプレイ停止", "リプレイ管理を開く", "リプレイ管理", "リプレイ", "曲名またはアーティストを検索", "リプレイファイル", "更新",
        "再生", "削除", "クリア", "失敗", "非対応", "リプレイなし", "保存または再生できる記録がありません。", "ゲームスレッドに追加しました。",
        "リプレイを保存しました", "リプレイを読み込めません", "閉じる", "前へ", "次へ", "リプレイ詳細", "記録日時", "進行度", "入力", "速度", "種類", "レベルパス",
        "公式レベル", "カスタムレベル", "このリプレイを削除しますか？", "キャンセル", "ページ", "アーティスト",
        "リプレイ名", "名前を保存", "名前を保存しました", "元の曲名", "一覧へ戻る", "レベルプレビュー", "カバーまたは試聴音源がありません");
}
